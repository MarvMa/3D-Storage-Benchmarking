...
class FastAPIUser(HttpUser):
    ...
    def upload_file(self):
        """Upload a file (POST to /store or whatever your upload route is)."""
        files = {"file": ("test_data.bin", self.file_data)}
        response = self.client.post("/store", files=files)

        if response.status_code == 200:
            # Suppose your endpoint returns JSON with an object_id
